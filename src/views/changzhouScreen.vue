      ...
      <template v-slot:content_main_left>
        <div class="content_left">
          <div class="total_data">
            ...
            <revenueCom :totalRevenue="yearRevenueAmount" :totalPercent="+yearDiffRate" :monthRevenue="monthRevenueAmount" :monthPercent="monthDiffRate" :dailyRevenue="dayRevenueAmount" :dailyPercent="dayDiffRate"></revenueCom>
          </div>
          <div class="asys_data">
      ...
      <template v-slot:content_main_right>
        <div class="content_right">
          <div class="weather_box">
            ...
            <changzhouWeather :date="yesterdayDate" loaction="常州市金坛区" :weather="weatherCondition" :temperatureMax="temperatureMax" :temperatureMin="temperatureMin" :aqi="AQI" :pm25="PM25" :humidity="humidity" :windDirection="windDirection" :windLevel="windLevel" :quality="quality" :qualityLevel="qualityLevel" :suggestion="suggestion"></changzhouWeather>
          </div>
          <div class="sales_box">
...
export default {
  ...
  components: {
    myLayout, subTitle, revenueCom, passagerAndRevenue, hotelData, dailyPassengerAndRevenue, changzhouMap, ticketNumberPie, touristScale, changzhouWeather, salesCount, passengerSource
  },
  data () {
    return {
      // 数据分析
  ...
  methods: {
    ...
    // 获取酒店数据总览
    getGrouPTotalData () {
      getTotalData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    // 获取数据分析数据-----------------------------------------------------------
    getGroupAnalyData () {
      getAnalyData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    getGroupPassengersData () {
      getPassengersData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    getGroupTicketsrsData () {
      getTicketsrsData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    getGroupTouristsData () {
      getTouristsData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    // 渠道销售统计
    getGroupChannelData () {
      getChannelData(2, this.yesterdayDate).then(res => {
        if (res) {
    ...
    // 酒店数据
    getGroupHotelData () {
      getHotelData(2, this.yesterdayDate).then(res => {
        if (res) {
